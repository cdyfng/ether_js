...
function watchTokenTransfers() {
  ...
  tokenContract.events.Swap(options, async (error, event) => {
    ...
    if(event.returnValues.amount1In==0 && event.returnValues.amount0Out == 0){
      if(event.returnValues.amount1Out / 1e18 >= 3){
          cmd.runSync('say big sell');
      }
      //sell 
    }else if(event.returnValues.amount0In==0 && event.returnValues.amount1Out == 0){
      //buy
    ...
    console.log(
      ...
      "  Found Swap from " +
        ...
        " " +
      event.returnValues.amount0In / 1e9 +  " "  +  
      event.returnValues.amount1In / 1e18 +  " " +
      event.returnValues.amount0Out / 1e9 +  " "  +  
      event.returnValues.amount1Out / 1e18 +  " " +
      "\n"
    );
    if ( event.returnValues.sender == '0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D' && event.returnValues.to == '0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D'){
